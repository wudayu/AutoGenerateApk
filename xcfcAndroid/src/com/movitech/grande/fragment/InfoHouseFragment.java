 ...
 * @version: 1.0
 * @Created Time: Jul 2, 2014 8:11:53 PM
...
public class InfoHouseFragment extends BaseFragment {
	...
	public void reDoLoad() {
		if (!isLoading) {
			...
			showProgressDialog();
			lvHouseInfo.resetVar();
			lvHouseInfo.resetFootView();
			lvHouseInfo.addOnScrollListener();
			doLoadData();
		}
	}
	@Background
	...
	void doLoadBanner(){
		resultHouseBanner = netHandler.postForGetInfoBanner(NewsCatagoryId.CATAGORY_ID_POLICY);

		if (null == resultHouseBanner) {
			goBackMainThreadBanner(getString(R.string.error_server_went_wrong), false);
			return;
		}

		if (resultHouseBanner.getResultSuccess() == false) {
			goBackMainThreadBanner(resultHouseBanner.getResultMsg(), false);
			return;
		}

		houseActivitiesBanner = resultHouseBanner.getInfoBanner();
        
		goBackMainThreadBanner(resultHouseBanner.getResultMsg(), true);
	}
	...
	private void initViews() {
		...
		lvHouseInfo.addHeaderView(viewBannerHeader);
		houseInfoLoadMore = getActivity().getLayoutInflater().inflate(
				R.layout.item_loading, null);
		houseInfoLoadMore.setOnClickListener(null);
		lvHouseInfo.addFooterView(houseInfoLoadMore);
		
		lvHouseInfo.setOnScrollToEdgeCallBack(new OnScrollToEdgeCallBack() {
	...
	void doLoadDataAndBindData() {
		isLoading = true;
		resultHouse = netHandler.postForGetInfoesResult(1, NewsCatagoryId.CATAGORY_ID_HOUSE, "", "");
		...
		if (resultHouse.getResultSuccess() == false) {
			goBackMainThread(resultHouse.getResultMsg(), false);
			return;
		}

		houseActivities = resultHouse.getPageResult().getInfoes();
		houseTotalPage = resultHouse.getPageResult().getPageNo();

		goBackMainThread(resultHouse.getResultMsg(), true);
	}
	...
	void doBindData() {
		...
		houseAdapter = new InfoListAdapter(getActivity().getApplicationContext(), houseActivities,
				R.layout.item_listview_fragment_info, imageUtils);
		lvHouseInfo.setAdapter(houseAdapter);
		lvHouseInfo.setTotalPageCount(houseTotalPage);
		lvHouseInfo.setCurrentPage(1);

		lvHouseInfo.setOnItemClickListener(new OnItemClickListener() {
			@Override
	...
	@UiThread
	 void initSlideViewPage() {
		if (houseActivitiesBanner == null) {
			return;
		}
		List<View> views = new ArrayList<View>();
		loader = new ImageDownLoader(getActivity().getApplicationContext());
		int bannerCount = (Constant.BANNER_COUNT > houseActivitiesBanner.length ? houseActivitiesBanner.length : Constant.BANNER_COUNT);
		for (int i = 0; i < bannerCount; ++i) {
			final XcfcInfoBanner infoBanner = (XcfcInfoBanner)houseActivitiesBanner[i];
			...
			imageUtils.loadHouseBannerImage(infoBanner.getBannerSrc(), imageViewBanner);
//			Bitmap bitmap = null;
...
//				imageViewBanner.setBackgroundResource(R.drawable.default_info_banner);
...
//			}
	...
	@Background
	void loadNewForHouse() {
		isLoading = true;
		houseCurrPage = lvHouseInfo.getCurrentPage() + 1;
		resultHouse = netHandler.postForGetInfoesResult(houseCurrPage, NewsCatagoryId.CATAGORY_ID_HOUSE, "", "");
		boolean illegal = (resultHouse == null || !resultHouse.getResultSuccess() || resultHouse
	...
	void addHouseItems() {
		dismissProcessingDialog();
		houseAdapter.addItems(resultHouse.getPageResult().getInfoes());
		lvHouseInfo.setCurrentPage(houseCurrPage);
		isLoading = false;
	}
